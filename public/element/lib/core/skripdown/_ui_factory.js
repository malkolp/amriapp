...
window._ui_factory = {
    __general : {
        ...
        compact_els : function (parentTag, elements) {
...
window._card = {
    ...
    remove : function (id) {
        ...
        const par_ = node.parentNode;
        par_.removeChild(node);
    }
}
...
window._navcard = {
    ...
    render : function (input) {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            this.memory[items[i].id]['plus']  = 0;
            this.memory[items[i].id]['minus'] = 0;
        }
    },
    update : function (id, value) {
        const stat     = this.memory[id][1];
        stat.innerText = value;
    },
    success : function (id, value=1) {
        const uif_lib  = _ui_factory._navcard;
        const stat     = this.memory[id][2];
        const plus     = this.memory[id]['plus'] + value;
        const minus    = this.memory[id]['minus'];
        if (plus > minus) {
            console.log('success plus ['+plus+' > '+minus+']');
            stat.innerHTML = '+'+(plus - minus);
        }
        else if (plus === minus) {
            _navcard.hide(id);
        }
        else {
            console.log('success minus ['+plus+' < '+minus+']');
            stat.innerHTML = '-'+(minus - plus);
        }
        this.memory[id]['plus'] = plus;

        uif_lib.success(stat);
    },
    danger : function (id, value=1) {
        const uif_lib  = _ui_factory._navcard;
        const stat     = this.memory[id][2];
        const plus     = this.memory[id]['plus'];
        const minus    = this.memory[id]['minus'] + value;
        if (plus > minus) {
            console.log('danger plus ['+plus+' > '+minus+']');
            stat.innerHTML = '+'+(plus - minus);
        }
        else if (plus === minus) {
            _navcard.hide(id);
            ...
            console.log('danger minus ['+plus+' < '+minus+']');
            stat.innerHTML = '-'+(minus - plus);
        }
        this.memory[id]['minus'] = minus;
...
window._formfield = {
    render : function (input) {
        ...
        for (let i = 0; i < input.fields.length; i++) {
            ...
            else if (input.fields[i].type === 'submit') {
                uif_gen.append_els(ctr, [temp[0]]);
            }
            else
