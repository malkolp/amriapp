...
<head>
    ...
    <script src="{{asset(env('LIB_PATH').'core/skripdown/_sidebar.js')}}"></script>
    <script src="{{asset(env('LIB_PATH').'core/skripdown/_element_tagger.js')}}"></script>
    <script src="{{asset(env('LIB_PATH').'core/skripdown/_school.js')}}"></script>
    <script>@include('root.token')</script>
    <script>
        const level=_data.level,student=_data.student,veryfied=_data.veryfied,unveryfied=_data.unveryfied;
            ...
            <nav class="sidebar-nav">
                <ul id="sidebarnav">
                    ...
                    <li class="sidebar-item">
                        ...
                        <ul aria-expanded="false" class="collapse first-level base-level-line" id="list-pendaftaran">
                            @if (sizeof($levels) > 0)
                                @foreach($levels as $level)
                                    <li class="sidebar-item" id="list-pendaftaran-{{$level->id}}">
                                        <a href="{{url('/manage/'.$level->name)}}" class="sidebar-link">
                                            <span class="hide-menu">{{strtoupper($level->name)}}</span>
                                        </a>
                                    </li>
                                @endforeach
                            @else
                                <li class="sidebar-item" data-empty="1">
                                    <a href="#" class="sidebar-link">
                                        <span class="hide-menu text-muted">tidak ada</span>
                                    </a>
                                </li>
                            @endif
                        </ul>
                    ...
                    <li class="sidebar-item">
                        ...
                        <ul aria-expanded="false" class="collapse first-level base-level-line" id="list-data">
                            @if (sizeof($levels) > 0)
                                @foreach($levels as $level)
                                    <li class="sidebar-item" id="list-data-{{$level->id}}">
                                        <a href="{{url('/data/'.$level->name)}}" class="sidebar-link">
                                            <span class="hide-menu">{{strtoupper($level->name)}}</span>
                                        </a>
                                    </li>
                                @endforeach
